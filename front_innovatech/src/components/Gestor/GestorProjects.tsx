    ...
    <div className="up-modal-overlay" onClick={onClose}>
      <div className="up-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="up-modal-subtitle">
          <Users size={15} />
          <span>{colaboradores.length} {colaboradores.length === 1 ? 'colaborador' : 'colaboradores'}</span>
        </div>
        <div className="up-modal-members">
          {colaboradores.length === 0 ? (
            <p className="up-modal-empty">Sin colaboradores asignados.</p>
          ) : colaboradores.map(c => (
    ...
    <div className="user-projects">
      <header className="up-header">
        <div className="d-flex align-items-center gap-2">
          <FolderKanban size={26} style={{ color: '#8b5cf6' }} />
          <div>
            <h1 className="up-title">Mis Proyectos</h1>
            <p className="up-subtitle">Proyectos que gestionas — ve el tablero de tareas de tu equipo.</p>
          </div>
        </div>
      </header>
            ...
            return (
              <Col key={p.id} xs={12} sm={6} xl={4}>
                <Card
                  ...
                  onClick={() => navigate(`/gestor/proyectos/${p.id}`)}
                  style={{ cursor: 'pointer' }}
                >
                  <Card.Body className="p-4">
                    <div className="d-flex justify-content-between align-items-start mb-3">
                      <div className="up-card-icon" style={{ background: 'rgba(139,92,246,0.1)', color: '#8b5cf6' }}>
                        <FolderKanban size={20} />
                      </div>
                      <Badge className="up-estado-badge" style={{ backgroundColor: cfg.bg, color: cfg.color }}>
                        {cfg.label}
                      </Badge>
                    </div>
                    ...
                    <div className="up-card-footer">
                      <button className="up-miembros-btn" onClick={e => abrirModal(e, p)}>
                        <Users size={14} />
                        {colaboradores.length} {colaboradores.length === 1 ? 'colaborador' : 'colaboradores'}
                      </button>
                      <span className="up-ver-tablero" style={{ color: '#8b5cf6' }}>
